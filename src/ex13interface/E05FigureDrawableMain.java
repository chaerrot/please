...
public class E05FigureDrawableMain
{
	...
	{
		/*
		 부모 인터페이스의 참조변수로 자식 객체를 참조하는 이질화 형태
		 */
		IFigure figure = new Circle(30);
		//오버라이딩되었으므로 별도의 형변환 없이 즉시 호출 가능
		figure.area("원");
		/*
		 IFigure 인터페이스에는 area() 추상 메서드만 정의되어있으므로
		...
		draw.draw("삼각형");
		/*
		 IDrawable 인터페이스는 draw()추상메서드만 정의되어있으므로
		 area()를 호출하려면 다운캐스팅해야 한다.
		 */
