...
/*
 ...
 오버로딩: 메서드명은 같으나 매개변수의 개수, 타입, 순서가
 	다른 경우를 말한다. 즉, 서로 다른 메서드이므로 상속 받은
...
class C extends B {
	/*
	 ...
	 정의된 반환형, 메서드명, 매개변수의 개수가 모두 동일하다.
	 */
	@Override
	public void rideMethod()
	{
	...
	/*
	 따라서 C 클래스로 객체를 생성하면 오버라이딩된 메서드는
	 모두 가려지게되므로 C 클래스에 정의된 메서드 하나만
	 남게 된다. 오버로딩된 메서드는 서로 다르므로 3개 모두
	 존재하게 된다.
	 */
}
...
public class E07RideAndLoad01
{
	...
	{
		/*
		 부모 타입의 참조변수로 자식 객체를 참조할 수 있으므로
		 아래 문장은 모두 성립한다.
		 */
		A ref1 = new C(); //O
		B ref2 = new C(); //O
		...
		/*
		 참조변수에 상관 없이 항상 최하위에 오버라이딩된 메서드가
		 호출된다. 즉 아래 부분은 모두 C 클래스에 생성된 rideMethod()
		 메서드가 호출된다.
		 */
		...
		/*
		 ...
		 따라서 접근범위는 C 객체 전체가 되므로 아래의 호출 문장은
		 모두 실행된다.
		 */
		...
		/*
		 ...
		 참조가 가능하다. 부모 타입의 객체를 자식 타입으로 변경한다.
		 클래스가 상속 관계에 있기 때문에 가능하다.
		 */
		B refId2 = (B)refId1;
		C refId3 = (C)refId1;
