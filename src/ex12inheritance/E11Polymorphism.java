...
/*
 다형성(Polymorphism)
 	...
 	특성을 말한다. 단, 객체 간에는 상속 관계가 있어야 한다.
 	
 	- 부모의 참조변수로 자식 객체를 참조
 	1. 이 경우 부모타입의 참조변수는 부모로부터 상속 받은 멤버까지만
 		접근할 수 있다.
 	2. 자식에서 오버라이딩한 메서드가 우선 호출된다.
 	3. 자식에서 새롭게 정의한 멥버는 접근할 수 없다.
 	4. 이런 경우를 이질화(Heterogeneous: 헤테로지니어스)라고 한다.
 	
 	- 같은 타입의 참조변수로 객체 참조
 	1. 객체 전체를 접근할 수 있다. 즉 클래스의 일반적인 규칙이 적용된다.
 	2. 이 경우를 동질화(Homogeneous)라고 한다.
 */
class Parent {
	int parentMember;
	void parentMethod() {
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		/*
		 이질화: 부모 타입의 변수로 자식 객체를 참조하는 것.
		 	이 경우 자식에서 새롭게 정의한 멤버에는 접근할 수 없다.
		 	만약 접근해야한다면 자식 타입으로 강제형변환(다운캐스팅)
		 	해야 한다.
		 */
		...
		//parent1.childMember = 1; //접근불가 -> 부모 타입으로 자식 멤버에 접근할 수 없다.
		parent1.parentMethod(); //오버라이딩한 메서드이므로 자식 쪽이 호출된다.
