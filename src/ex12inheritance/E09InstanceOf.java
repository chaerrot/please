...
/*
 instanceof 연산자
 	...
 	반환한다.
 	- 객체 간의 형변환이 되려면 반드시 두 클래스 간에 상속관계가
 	있어야 한다.
 	- 부모클래스의 참조변수로 자식 객체를 참조할 수 있으므로,
...
//	static void 
	...
	/*
	 ...
	 - Box 타입으로 인자를 받게 되면 자동으로 업캐스팅(자동형변환)이 되기
	 때문에 자식클래스의 멤버메서드를 호출할 수 없게 된다.
	 - 이 때 자식의 멤버메소드를 호출하기 위해서 다운캐스팅(강제형변환)이
	 필요하다.
	...
	public static void main(String[] args)
	{
		...
		wrapBox(box1);
		wrapBox(box2);
		wrapBox(box3); //각 타입의 인자를 부모타입인 Box로 한 번에 받을 수 있다.
		
		String str = new String("나는누구?");
		//wrapBox(str); //String 객체와는 상속 관계가 없으므로 호출 불가능
