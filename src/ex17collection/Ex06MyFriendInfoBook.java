...
class HighFriend extends Friend {
	...
	/*
	 고딩친구의 전체 정보를 출력하기 위해 부모에서 정의된 메서드를
...
public class Ex06MyFriendInfoBook
{
	...
	public static void main(String[] args)
	{
		...
		FreindInfoHandler handler = new FreindInfoHandler(100);
...
class FriendInfoHandler {
	...
	public void addFriend (int choice) {
		...
		if (choice ==1) { //고딩친구 입력 후 추가
			...
			//고딩친구 객체를 만든 후 컬렉션에 add한다. 인덱싱은 필요 없다.
			...
			list.add(new UnivFriend(iName, iPhone, iAddr, iMajor));
		}
	...
	/*
	 일반for문을 통해 컬렉션에 접근한다. 이 때에는 인덱스를 사용하므로
	 get()메서드를 활용한다. 컬렉션에 저장된 객체의 개수는 size()메서드를
	 통해 알 수 있다.
	 */
	...
	/*
	 ...
	 훨씬 더 간결해진다. 컬렉션에 저장된 객체의 개수만큼 알아서 반복한다.
	 */
	...
	public void searchInfo() {
		...
		/*
		 ...
		 반환되므로 주의해야 한다.
		 */
		Iterator<Friend> itr = lists.itreator();
		while (itr.hasNext()) {
			Friend fr = ite.next();
