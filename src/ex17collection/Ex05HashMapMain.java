...
public class Ex05HashMapMain
{
	...
	public static void main(String[] args)
	{
		...
		/*
		 출력하기
		 	1. 키 값을 알고 있을 때: get(키값) 메서드를 이용한다.
		 */
		...
		/*
		 	2. 키 값을 모를 때
		 		...
		 		이를 통해 Value를 얻어 올 수 있다.
		 	※ Map계열의 컬렉션은 처음부터 개선된 for문을 사용하는 것은
		 	불가능하다. 아래처럼 Key값을 먼저 얻어 온 후 사용해야 한다.
		 */
		Set<String> keys = map.keySet();
		System.out.println("[확장for문 적용]");
		for (String key : keys) {
			String value = map.get(key);
			System.out.println(String.format("%s: %s", keys, value));
		}
		
		/*
		 	3. 이터레이터를 통한 출력
		 		: 이터레이터를 통해 출력할 때도 keySet()을 통해
		 		먼저 key값을 얻어 온 후 반복 출력해야 한다.
		 */
		...
		/*
		 Value만 얻어 와야 할 때: values() 메서드 사용
		 */
