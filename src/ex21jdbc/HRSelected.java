	...
	public static void main(String[] args)
	{
		try {
			...
			if (con!=null) {
				...
				/*
				 1. 쿼리문작성: 작성 시 주의할 점은 줄바꿈을 할 때 앞뒤로
				 	스페이스를 삽입하는 게 좋다. 그렇지 않으면 서로 문장이
				...
				/*
				 3. 쿼리문을 전송한 후 실행한 결과를 ResultSet 객체를 통해
				 	반환 받는다. 해당 객체는 select쿼리를 실행했을 때만 사용한다.
				 */
				ResultSet rs = stmt.executeQuery(sql);
				/*
				 4. 반환된 결과를 개수만큼 반복한다. next()메서드는 출력할
				...
				while(rs.next()) {
					/*
					 ...
					 그와 동일하다. 각 컬럼에 접근 시 인덱스, 컬럼명 두 가지 모두
					 사용할 수 있다.
					 */
